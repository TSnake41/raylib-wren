import "raylib" for Raylib, Key, Color, Texture2D, Vector2, Rectangle, Text
...
while (!Raylib.windowShouldClose) {
  ...
  for (i in 0..(bunniesCount - 1)) {
    var bunny = bunnies[i]
    texBunny.draw(bunny.position.x, bunny.position.y, 0, 1, bunnies[i].color)
  }

  Rectangle.draw(0, 0, screenWidth, 40, Color.black, false)
  Text.draw(120, 10, "bunnies: %(bunniesCount)", 20, Color.green)
  Text.draw(320, 10, "batched draw calls: %(1 + (bunniesCount/maxBatchElements).ceil)", 20, Color.maroon)
